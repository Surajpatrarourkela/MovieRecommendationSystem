...
if st.session_state['logged_in']:
    ...
    if st.button("Recommend"):
        ...
        for i in range(len(names)):
            st.subheader(f"**{names[i]}**")  # Bold Movie Name
            if posters[i]:
                st.image(posters[i])
            # Star Rating System
            rating = st.select_slider(f"Rate **{names[i]}**", options=[1, 2, 3, 4, 5], value=3)
            st.text(f"Rating: **{rating}** / 5")  # Display value next to the star rating
